...
if 'token' not in st.session_state:
    ...
    if "access_token" not in st.session_state:
        st.session_state['access_token'] = utils.get_iam_oidc_token(token['id_token'])['idToken']
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                response = utils.get_queue_chain(prompt,user_email,st.session_state['conversationId'], st.session_state['parentMessageId'], st.session_state['access_token'])
